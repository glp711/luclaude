...
const FROM_ADDR = "perfumesdeambientedecor <onboarding@resend.dev>";
...
export async function sendOrderPaidEmail(input: {
  ...
  await resend.emails.send({
    from: FROM_ADDR,
    to: toEmail,
    subject: `Pedido #${input.orderNumber} confirmado · perfumesdeambientedecor`,
    html: `
      <div style="font-family:'Helvetica Neue',Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;background:#f5ecdc;color:#2d2924">
        ...
        <div style="background:#faf4e9;border:1px solid #ebe0cc;border-radius:16px;padding:20px;margin-bottom:20px">
          ...
          <p style="margin:0;color:#6e655a;font-size:14px">A perfumesdeambientedecor já está preparando seu pedido com carinho. Você receberá outro e-mail quando o produto for postado, com o código de rastreio.</p>
        </div>
